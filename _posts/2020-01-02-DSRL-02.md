...
\gamma를 조절해서 현재의 reward에 가중치를 줄지 아니면 미래를 내다볼지 정할 수 있다. Return 과 Value Function를 예시를 들어서 차이점을 설명하자면 a와 b 가 스타크래프트를 할 때 매 판 결과가 있을 것이다. 이 때 매 판의 결과가 Return이다. 그러나 a와 b가 경기를 무수히 많이 해서 승률을 낸다면 이것은 수렴을 할 것이다. 이 때 승률이 value function이다. 결국 return의 기댓값이 value가 된다.
